"""
KIS Trend-ATR Trading System - 모의투자 전용 설정

═══════════════════════════════════════════════════════════════════════════════
⚠️ 이 파일은 PAPER(모의투자) 모드 전용 설정입니다.
   실계좌에는 절대 적용되지 않습니다.
═══════════════════════════════════════════════════════════════════════════════

★ 목적:
  - 모의투자 환경에서 전략 테스트
  - 실계좌보다 약간 공격적인 설정 가능
  - 다양한 파라미터 실험

★ 안전 장치:
  - API URL이 항상 모의투자 서버로 고정
  - REAL 모드 관련 설정은 무시됨

작성자: KIS Trend-ATR Trading System
버전: 2.0.0
"""

from config.settings_base import *

# ═══════════════════════════════════════════════════════════════════════════════
# 모의투자 전용 설정 (Base 오버라이드)
# ═══════════════════════════════════════════════════════════════════════════════

# ★ 모의투자 API URL 강제 고정
KIS_BASE_URL = PAPER_API_URL

# ★ 실계좌 관련 설정 강제 비활성화
ENABLE_REAL_TRADING = False
EXECUTION_MODE = "PAPER"


# ═══════════════════════════════════════════════════════════════════════════════
# 리스크 관리 (모의투자용 - 약간 여유롭게)
# ═══════════════════════════════════════════════════════════════════════════════

# 일일 최대 손실 비율 (%) - 모의투자는 5%까지 허용
DAILY_MAX_LOSS_PERCENT = 5.0

# 일일 최대 거래 횟수 - 모의투자는 5회까지 허용
DAILY_MAX_TRADES = 5

# 연속 손실 허용 횟수 - 모의투자는 3회까지 허용
MAX_CONSECUTIVE_LOSSES = 3

# 누적 드로다운 한도 (%) - 모의투자는 20%까지 허용
MAX_CUMULATIVE_DRAWDOWN_PCT = 20.0


# ═══════════════════════════════════════════════════════════════════════════════
# 전략 파라미터 (모의투자용 - 실험 가능)
# ═══════════════════════════════════════════════════════════════════════════════

# 손절/익절 배수 (실험용으로 조정 가능)
# ATR_MULTIPLIER_SL = 2.0  # 기본값 사용
# ATR_MULTIPLIER_TP = 3.0  # 기본값 사용


# ═══════════════════════════════════════════════════════════════════════════════
# 주문 설정 (모의투자용)
# ═══════════════════════════════════════════════════════════════════════════════

# 1회 주문 수량 (모의투자는 더 큰 수량 가능)
# ORDER_QUANTITY = 10  # 필요시 오버라이드


# ═══════════════════════════════════════════════════════════════════════════════
# 갭 보호 (모의투자에서도 필수 유지)
# ═══════════════════════════════════════════════════════════════════════════════

# ★ 갭 보호는 모의투자에서도 반드시 활성화
ENABLE_GAP_PROTECTION = True


# ═══════════════════════════════════════════════════════════════════════════════
# 검증 함수
# ═══════════════════════════════════════════════════════════════════════════════

def validate_paper_settings() -> tuple[bool, list[str]]:
    """모의투자 설정 검증"""
    errors = []
    
    # 모의투자 URL 확인
    if "openapivts" not in KIS_BASE_URL:
        errors.append("⚠️ 모의투자 URL이 아닙니다!")
    
    # 실계좌 설정 차단 확인
    if ENABLE_REAL_TRADING:
        errors.append("⚠️ ENABLE_REAL_TRADING이 True입니다!")
    
    if EXECUTION_MODE != "PAPER":
        errors.append(f"⚠️ EXECUTION_MODE가 {EXECUTION_MODE}입니다!")
    
    return (len(errors) == 0, errors)


def print_paper_settings_summary() -> str:
    """모의투자 설정 요약"""
    return f"""
═══════════════════════════════════════════════════════════════
🟡 KIS Trend-ATR Trading System - 모의투자 설정
═══════════════════════════════════════════════════════════════
[모드]
  실행 모드: 🟡 PAPER (모의투자)
  API URL: {KIS_BASE_URL}

[리스크 관리]
  일일 손실 한도: {DAILY_MAX_LOSS_PERCENT}%
  일일 최대 거래: {DAILY_MAX_TRADES}회
  누적 드로다운 한도: {MAX_CUMULATIVE_DRAWDOWN_PCT}%

[안전 장치]
  갭 보호: {'✅ ON' if ENABLE_GAP_PROTECTION else '❌ OFF'}
  단일 인스턴스 강제: {'✅ ON' if ENFORCE_SINGLE_INSTANCE else '❌ OFF'}
  장 운영시간 강제: {'✅ ON' if ENFORCE_MARKET_HOURS else '❌ OFF'}
═══════════════════════════════════════════════════════════════
⚠️ 이 설정은 모의투자 전용입니다.
   실계좌에서는 settings_real.py가 적용됩니다.
═══════════════════════════════════════════════════════════════
"""
