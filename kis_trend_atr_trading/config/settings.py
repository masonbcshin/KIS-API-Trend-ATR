...
# 갭 리스크 완화 활성화 여부
# ★ 감사 보고서 지적: 멀티데이 전략에서 갭 보호 OFF는 자살 행위
# ★ 수정: 기본값 True로 변경 (필수)
...
# 누적 드로다운 제어 (신규)
# ════════════════════════════════════════════════════════════════
# ★ 감사 보고서 지적: 일일 손실 한도는 복리 누적 손실을 막지 못함
# ★ 해결: 누적 드로다운 제어 추가

# 최대 누적 드로다운 허용 비율 (%)
# 초기 자본 대비 누적 손실이 이 비율을 초과하면 킬 스위치 발동
MAX_CUMULATIVE_DRAWDOWN_PCT = 15.0

# 누적 드로다운 경고 비율 (%)
# 이 비율 도달 시 텔레그램 경고 발송
...
# 체결 확인 최대 대기 시간 (초)
# ★ 수정: 45초로 연장 (기존 30초) - 유동성 낮은 종목 대비
ORDER_EXECUTION_TIMEOUT = 45
...
# 부분 체결 시 자동 재주문 활성화
ENABLE_PARTIAL_FILL_RETRY = True

# 부분 체결 재주문 최대 횟수
PARTIAL_FILL_MAX_RETRIES = 3
...
# 실행 주기 설정 (신규)
# ════════════════════════════════════════════════════════════════
# ★ 감사 보고서 지적: 60초 실행 간격으로 손절 기회 놓침

# 기본 실행 간격 (초)
# ★ 권장: 손절 감시 전용 모드에서는 더 짧은 간격 필요
DEFAULT_EXECUTION_INTERVAL = 60

# 손절선 근접 시 실행 간격 (초)
# 손절선까지 70% 이상 도달 시 이 간격으로 전환
NEAR_STOPLOSS_EXECUTION_INTERVAL = 15
