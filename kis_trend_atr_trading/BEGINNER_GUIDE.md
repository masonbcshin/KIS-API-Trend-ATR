...
> - Mac/Linux에서는 `python3 --version`으로 시도해보세요. (python 대신 python3 사용)

> 💡 **Mac/Linux 사용자**: 이후 명령어에서 `python` 대신 `python3`를 사용하세요!
...
#### 왜 필요한가요?
가상환경은 **프로그램 전용 방**을 만드는 거예요.
다른 Python 프로그램과 섞이지 않게 해줍니다.
...
활성화되면 터미널 앞에 `(venv)`가 표시돼요:
```
(venv) C:\Users\내이름\kis_trend_atr_trading>
```

> 💡 **팁**: 프로그램을 실행할 때마다 가상환경을 먼저 활성화하세요!
...
터미널에서 프로그램 폴더로 이동한 후 (가상환경 활성화된 상태에서):
...
프로그램 폴더 안에 뭐가 있는지 알면 좋아요:

```
kis_trend_atr_trading/
├── main.py              ← 기본 실행 파일
├── main_v2.py           ← 고급 실행 파일 (DEV/PROD 분리)
├── report_sender.py     ← 일일 리포트 전송
...
│
├── data/                ← 데이터 저장 폴더
│   ├── positions.json   ← 포지션 정보 (자동 저장)
│   └── daily_trades.json← 당일 거래 기록
│
├── logs/                ← 로그 파일 폴더
│
├── tests/               ← 테스트 코드 폴더
...
- [x] Python이 설치되어 있나요?
- [x] 가상환경이 활성화되어 있나요? (터미널 앞에 `(venv)` 표시)
...
║   평일 (월~금요일)                                               ║
║   - 정규장: 오전 9시 ~ 오후 3시 30분                             ║
║   - 이 프로그램 거래 시간: 오전 9시 ~ 오후 3시 20분              ║
║     (동시호가 시간을 피하기 위해 10분 일찍 마감)                 ║
║                                                                   ║
║   휴일 (토, 일, 공휴일)                                          ║
...
> 💡 **참고**: 프로그램이 휴장일을 자동으로 판단해요. `utils/market_hours.py` 파일에 휴장일 목록이 있습니다.
...
리포트를 받으려면 거래 데이터가 있어야 해요.

**방법 1: 샘플 파일 복사**
```bash
# data 폴더에 샘플 파일이 있어요
cp data/trades_sample.csv data/trades.csv
```

**방법 2: 직접 모의투자 실행**
모의투자를 실행하면 거래 데이터가 자동으로 저장돼요.
...
이 프로그램은 **보유 중인 포지션 정보를 자동으로 저장**해요.

**왜 필요한가요?**
- 프로그램이 갑자기 꺼져도 "내가 뭘 사고 있었는지" 기억해요
- 다시 켜면 이어서 거래할 수 있어요

**저장 위치:**
```
data/positions.json    ← 현재 보유 포지션
data/daily_trades.json ← 당일 거래 기록
```

### 로그 파일

프로그램이 실행되면서 발생하는 모든 기록이 저장돼요.
...
> 💡 **팁**: 문제가 생겼을 때 로그 파일을 보면 원인을 찾기 쉬워요!
...
2. 가상환경이 활성화됐는지 확인
   → 터미널 앞에 (venv)가 있나요?
   → 없으면: venv\Scripts\activate (Windows)
