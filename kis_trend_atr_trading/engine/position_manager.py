...
class ManagedPosition:
    ...
    def __post_init__(self):
        if not self.created_at:
            self.created_at = datetime.now(KST).strftime("%Y-%m-%d %H:%M:%S")
    ...
    def update_unrealized(self, current_price: float) -> None:
        ...
        try:
            ...
            # KST 기준 날짜로 계산하여 타임존 차이로 인한 오류 방지
            self.holding_days = (datetime.now(KST).date() - entry_dt.date()).days + 1
        except:
            pass
...
class PositionManager:
    ...
    def open_position(
        ...
        with self._lock:
            ...
            # 포지션 생성
            position_id = f"P{datetime.now(KST).strftime('%Y%m%d%H%M%S')}_{stock_code}"
    ...
    def close_position(
        ...
        with self._lock:
            ...
            position.exit_date = datetime.now(KST).strftime("%Y-%m-%d")
            position.exit_time = datetime.now(KST).strftime("%H:%M:%S")
            ...
            try:
                ...
                position.holding_days = (datetime.now(KST) - entry_dt).days + 1
            except:
                pass
